...
本 skill 本質上需要一個參考網站才能運作（URL 是輸入）。因此由調度器（frontend-director 或同等）載入的條件是：**使用者提供了參考網站／明確表示要模仿某個站的風格**（如「做成像 X 那樣」「分析 X 的設計」）。一般創意專案（使用者只描述產品與願景、未提供參考站）**不載入本 skill**，由 `impeccable`＋`ui-ux-pro-max` 承擔設計方向；若無參考站仍需要品味基準，以 [creative-tier.md] 的等級標準與 ui-ux-pro-max 的模式資料取代。
