...
本 skill 只在以下情境由調度器（frontend-director 或同等）載入：
1. 使用者**明確要求**模仿某品牌／某網站的設計語言（「做成像 X 那樣」「參考 X 的風格」）；或
2. 使用者**明確要求**建立可重複使用的設計語言／設計系統 skill。

一般創意專案（使用者只描述產品與願景，未指定模仿對象）**不載入本 skill**，由 `impeccable`（craft）與 `taste`（品味逆向）承擔設計方向。當被調度時，遵循 [creative-tier.md] 的創意等級；本 skill 生成設計語言時，「subtle-by-default」僅適用於 Tier 1，Tier 2／3 應把「大膽」寫進設計模型的默認值。
