...
## 與創意設計的關係（交互邏輯為本，不限制視覺）

本 skill 的定位是「在做 UI 前先設計好交互邏輯」：流程完整性、導覽結構、狀態與錯誤、鍵盤與觸控。它**不評判視覺風格**，也不限制創意交互：

- **創意互動加分不扣分**：拖曳、手勢、cursor 互動、動效敘事、遊戲化回饋等創意交互手段，只要主要流程仍可完成、狀態仍可感知，就應**額外加分**，不受防缺陷扣分制限制。
- **評分閘門只評交互邏輯**：scoring.md 的 100 分制檢查的是「任務能不能完成、迷不迷路、錯誤能不能恢復」，不是「夠不夠克制」。
- 大膽的視覺與動效若讓某個狀態不夠清晰，修正的是「狀態可感知的呈現方式」，不是退回保守設計。
