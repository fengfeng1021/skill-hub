...
- ✅ 必须：Step 2 和 Step 5 **无条件执行**。复杂度判定仅影响 Step 3（ChatGPT 推理）是否跳过
...
```
用户需求 → [1.Claude 规划] → [2.Kimi 搜索] →{复杂?}→ [3.ChatGPT 推理] → [4.Claude 合成] → [5.ChatGPT 审查] → [6.Claude 修复] → 最终产出
                                              └── 简单: 跳过 ──┘
```
...
| 3 | ChatGPT | 深度推理 (条件) | `直接给出完整的分析。不需要搜索新资料，基于已有信息推理。` |
| 5 | ChatGPT | 交叉审查 | `请逐一审查以下内容，列出所有问题点并给出具体修改建议。不要重写整个方案。` |

## 复杂度判定 (Step 1)

**仅决定 Step 3 (ChatGPT 推理) 是否执行。不影响 Step 2 和 Step 5 的强制执行。**
...
Fallback: ChatGPT → Gemini → Claude
...
node skills/AgentChat-WebSubAgent/index.js --reason "prompt"    # ChatGPT 推理
