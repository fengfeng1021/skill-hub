...
> **職責邊界：本閘門不審查設計。** 它驗證的是程式品質（測試／lint／build／錯誤處理）、審查與安全風險——「視覺好不好看、大不大膽」不屬於本閘門的關卡，設計標準由 UI 階段的設計 skills 與創意等級負責。本閘門的檢查不得用來阻擋大膽的視覺或動效實作；若安全審查發現互動實作有風險，修正的是實作方式，不是退回保守設計。
