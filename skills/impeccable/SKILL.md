...
## 大膽導向（Bold-by-Default）與調度關係

被 frontend-director（或同等調度器）調度時：

- **bolder 不是選配**：Tier 2／3 專案（見 creative-tier.md）執行完本 skill 主流程後，必須接著執行 `bolder` 指令（或同等「放大膽」流程），把安全、平庸的版本當成中間產物，不是終點。
- **craft-floor 是品質底線，不是創意上限**：禁令清單防止「假品質」（假寫實、共用貼圖、遮掩缺陷）；它們不限制實驗性排版、破壞性網格、超比例字型、粗獷材質、動態構成等大膽手段。當禁令與「更大膽」衝突時，先檢查是否為「誠信問題」；是 → 遵守，否 → 大膽優先。
- **「克制」不是默認**：若成品能被人形容為「安全、保守、工整」而沒有其他記憶點，視為失敗，回到 bolder。
- 對真實性規則（photo-hero rule、honesty rule）：適用於事實性主張；非事實性的抽象、藝術、品牌構成不在此限（見 frontend-director creative-tier.md §與真實性規則的關係）。
